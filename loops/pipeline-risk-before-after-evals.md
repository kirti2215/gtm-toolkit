...
|---|---|
| **Risk understanding changes** | Feedback may confirm a hypothesis, reveal its cause, change its severity, or identify a different risk — the first diagnosis was right for the information available, but new evidence made it more precise |
...
**Scenario 2:** Feedback made the anticipated artifact obsolete. The diagnostic
call — the Loop's Pass 1 recommendation — worked exactly as intended. What became
unnecessary was the battle card that would have followed: the feedback revealed the
competitive motion had already shifted, so the artifact queued behind the diagnostic
call was no longer the right one.
