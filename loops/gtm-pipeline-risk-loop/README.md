...
Claude conversation to preserve deal context, incorporate new evidence, and revise
recommendations across passes — rather than treating each review as a fresh analysis
with no memory of what came before.

A Claude Loop is a structured cycle that maintains deal state across multiple passes,
updates its diagnosis when new evidence arrives, and decides when a pattern has
shifted enough to change the recommended action.
...
## Connection to the Win/Loss Pattern Loop

The Pipeline Risk Loop and the Win/Loss Pattern Loop are designed to operate in
sequence, not in parallel. The Win/Loss Loop is retrospective — it analyzes closed
deals, builds a pattern model over time, and tells you what has been driving wins
and losses in a segment. When a pattern reaches Matched Evidence confidence in the
Win/Loss Loop, that finding can inform how the Pipeline Risk Loop weights specific
risk types in the same segment. For example: if the Win/Loss Loop has confirmed
(under tested conditions) that deals with an IT Director as sole champion lose at
near-100% in competitive enterprise fintech evaluations, that validated pattern may
calibrate the Stakeholder/Champion Risk assessment for open deals in that segment —
raising the baseline risk rating for deals matching the same champion profile.

This calibration is a manual step, not automatic. The practitioner decides which
confirmed Win/Loss patterns are relevant to the current pipeline, and applies them
as prior knowledge when structuring the Pipeline Risk Loop input. Neither Loop
predicts open deal outcomes. The Win/Loss Loop tells you what has happened; the
Pipeline Risk Loop tells you where to focus attention right now.
